...
% Mini-batch size, Learning rate, Momentum, Dimensionality of embedding
% space, Number of units in hidden layer,  Standard deviation of the normal
% distribution sampled to get the initial weights.
batchsize = 100; 
learning_rate = 0.2;
momentum = 0.9;
numhid1 = 400; 
...
% randn() allocates values sampled randomly from N(0,1)
% number of random values to generate specified as matrix dimensions
...
% zeros() allocates null values
% number of null values to generate specified as matrix dimensions
...
% eye(arg) allocates an nxn identity matrix where arg is n
...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    % target batch is not a test batch, but a labelled output batch
    ...
    % COMPUTE DERIVATIVE.
    %% Expand the target to a sparse 1 x K vector
    % in the operation  below, expanded_target_batch is assigned values  
    % such that its i-th column is the j-th column of expansion_matrix, 
    % where j is the i-th value in target_batch, which is a vector. the 
    % values of target_batch are not words from the data, but the indices
    % for those words. values are from {0,1}.
    % the whole point of doing this is that multiplying the (transpose of?)
    % the result by a matrix containing every word in the vocab at the 
    % correct indices would create a matrix with each entry of 
    % target_batch in a separate column (row), and the position of each 
    % word inside its column would correspond to its index.
    expanded_target_batch = expansion_matrix(:, target_batch);
    %% Compute derivative of cross-entropy loss function.
    % this only computes the difference between what is output and what
    % should be output
    error_deriv = output_layer_state - expanded_target_batch;

    % MEASURE LOSS FUNCTION
    % CE is the cross entropy between the probability mass functions 
    % defined by expanded_target_batch and output_layer_state. cross 
    % entropy is a bit like a distance function in the space of pmfs. tiny
    % does not conform to the definition of cross entropy, but otherwise
    % we'd get errors for log(output_layer_state) when output_layer_state=0
    CE = -sum(sum(...
      expanded_target_batch .* log(output_layer_state + tiny))) / batchsize;
    count =  count + 1;
    
    % this_chunk_CE is actually the average CE for one chunk, where size of
    % chunk defined by show_training_CE_after. so there will be a jump
    % between last CE of a chunk and first CE of next chunk.
    this_chunk_CE = this_chunk_CE + (CE - this_chunk_CE) / count;
    trainset_CE = trainset_CE + (CE - trainset_CE) / m;
    fprintf(1, '\rBatch %d Train CE %.3f', m, this_chunk_CE);
    % mod(a,b) aka a%b in java
    ...
    % the last 2 terms of operation below are d_y/d_z for logistic neuron
    ...
    % in back_propagated_deriv_2, each row corresponds to a neuron from the
    % embedded layer, each column corresponds to an input
    ...
    if mod(m, show_validation_CE_after) == 0
      ...
      % stop training if overfitting starts harming 'true' performance
      if CE - min_validation_CE > 0.05
          force_end = true;
          fprintf(1, ' Stopped training after epoch %.3f\n', epoch);
          fprintf(1, ' because validation CE is now %.3f\n', CE);
          fprintf(1, ' whereas min is %.3f\n', min_validation_CE);
          break;
      end
...
fprintf(1, 'min validation CE %.3f\n', min_validation_CE); % my code
